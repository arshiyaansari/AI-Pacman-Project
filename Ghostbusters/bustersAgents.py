...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        finalAction = None
        ghost_positions = []
        #create list of all ghost positions with highest probability
        for posDist in livingGhostPositionDistributions:
            temp_pos = max(posDist, key=posDist.get)
            ghost_positions.append(temp_pos)

        for gpos in ghost_positions:
            #find distance to each ghost
            temp_dist = self.distancer.getDistance(pacmanPosition, gpos)
            for action in legal:
                succ_pos = Actions.getSuccessor(pacmanPosition, action)
                temp = self.distancer.getDistance(succ_pos, gpos)
                if temp_dist > temp:
                    finalAction = action
            return finalAction